...
const CACHE_KEY = 'portfolio_cache_v8';
...
async function getPortfolio(isSubpage = false) {
  ...
  try {
    ...
    let localRes = await fetch((inSub ? '../assets/data/data.json?v=8' : 'assets/data/data.json?v=8'), { cache: 'no-cache' });
    if (!localRes.ok) {
      localRes = await fetch((inSub ? 'assets/data/data.json?v=8' : '../assets/data/data.json?v=8'), { cache: 'no-cache' });
    }
