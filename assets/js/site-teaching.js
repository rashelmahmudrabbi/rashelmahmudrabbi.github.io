(async function () {
  ...
  const d = result.data || {};
  function initTeachingView(data) {
    ...
    const rolesList = (t.roles && t.roles.length) ? t.roles : ((data.teachingRoles && data.teachingRoles.length) ? data.teachingRoles : defaultRoles);
    const areasList = (t.areas && t.areas.length) ? t.areas : ((data.teachingAreas && data.teachingAreas.length) ? data.teachingAreas : defaultAreas);
    const philosophyText = t.philosophy || s.teaching_philosophy || data.teachingPhilosophy || defaultPhilosophy;
    const mentoringText = t.mentoringText || s.teaching_mentoring_text || data.teachingMentoringText || defaultMentoring;
    ...
    const areasCol = (arr, offset = 0) =>
      arr
        .map(
          (a, i) => `
      <div class="area-item">
        <div class="area-icon"><i class="bi ${areaIcons[(offset + i) % areaIcons.length]}"></i></div>
        <div>
          <div class="area-topic">${escapeHtml(a.topic || '')}</div>
          <div class="area-desc">${escapeHtml(a.description || a.desc || '')}</div>
        </div>
      </div>`
        )
        .join('');
  ...
  window.addEventListener('portfolio-updated', (e) => {
    if (e.detail && e.detail.data) {
      initTeachingView(e.detail.data);
    }
  });
})();
